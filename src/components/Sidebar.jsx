...
export function Sidebar() {
  ...
  return (
    <div className=" flex min-h-screen flex-col justify-self-start gap-10 bg-slate-900 border-2 border-violet-300/8 p-5 w-64 lg:w-85">
      ...
      <div className="flex flex-col justify-between h-full gap-10">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link to="/expense" className={`${pathname === "/expense" ? "flex gap-2 p-4 rounded-2xl ring-2 border-violet-500/20 bg-violet-500/20 text-violet-400 ": 
            "flex gap-2 p-4"}`}>
            <Wallet />
            <p className="text-amber-50">Despesas</p>
          </Link>

          <Link to="/dashboard" className={`${pathname === "/dashboard" ? "flex gap-2 p-4 rounded-2xl ring-2 border-violet-500/20 bg-violet-500/20 text-violet-400" : "flex gap-2 p-4"}`}>
            <ChartColumn />
        ...
        <div className="flex flex-col border-t-2 border-violet-300/20 pt-5 gap-2">
              <Link to="/profile" className={`${pathname === "/profile" ? "flex p-4 gap-2  rounded-2xl ring-2 border-violet-500/20 bg-violet-500/20 text-violet-400" : "flex gap-2 p-4"}`} >
                <User />
                <p className="text-amber-50">Perfil</p>
              </Link>
              <Link to="/login" className="flex gap-2  p-4">
                <LogOut />
